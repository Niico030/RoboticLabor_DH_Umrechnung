...
% 1) betrag von q0, q1, q2, q3 müssen <= 1 sein
% 2) q0^2+q1^2+q2^2+q3^2 muss = 1 ergeben

    for i=1: length(quaternionZeros)
        % Überprüfung der der Quaternionen |q1|, |q2|, |q3| <=1
        if abs(quaternionVektors(i,1)) <= 1  && abs(quaternionVektors(i,2)) <= 1   && abs(quaternionVektors(i,3)) <= 1
        else
            msg = 'Betrag der Quaternion muss kleiner als 1!\n - Bitte Eingabe der Quaternionen überprüfen';
